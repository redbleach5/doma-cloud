...
// ---- Environment ----
// ALWAYS force the test storage root to an absolute path under the project
// root. We MUST override any inherited STORAGE_LOCAL_ROOT (e.g. from a
// system shell profile) so tests never accidentally write to a real
// storage directory. The `??` was inconsistent with the DATABASE_URL
// override above — if a developer had STORAGE_LOCAL_ROOT set in their
// shell, tests would silently write there.
import { resolve } from "node:path";
const PROJECT_ROOT = resolve(import.meta.dir, "..");
process.env.DATABASE_URL = `file:${PROJECT_ROOT}/prisma/test.db`;
process.env.STORAGE_LOCAL_ROOT = `${PROJECT_ROOT}/storage-data-test`;
