...
describe("POST /api/files/[id]/share (create share)", () => {
  ...
  it("creates a share link for a file", async () => {
    const { response, data } = await callRoute<{ token: string; url: string; hasPassword: boolean; updated?: boolean }>(createShare, {
      method: "POST",
      params: { id: fileId },
      body: {},
  ...
  it("reuses the same token when sharing the same file again", async () => {
    const first = await callRoute<{ token: string; updated?: boolean }>(createShare, {
      method: "POST",
      params: { id: fileId },
      body: { label: "first" },
      cookies: { doma_session: token },
    ...
    const second = await callRoute<{ token: string; updated?: boolean }>(createShare, {
      method: "POST",
      params: { id: fileId },
      body: { label: "second", maxViews: 3 },
      cookies: { doma_session: token },
    });
    ...
    const { data: listed } = await callRoute<{ shares: { token: string; label: string | null; maxViews: number | null }[] }>(listShares, {
      method: "GET",
      params: { id: fileId },
      cookies: { doma_session: token },
    });
    expect(listed!.shares).toHaveLength(1);
    expect(listed!.shares[0]!.label).toBe("second");
    expect(listed!.shares[0]!.maxViews).toBe(3);
  });
...
describe("GET /api/files/[id]/share (list shares)", () => {
  ...
  it("lists existing shares for a file", async () => {
    await seedShare({ nodeId: fileId, createdBy: user.id, token: "tok1" });
    await seedShare({ nodeId: fileId, createdBy: user.id, token: "tok2" });
  ...
  it("does not leak shares for another user's file", async () => {
    const other = await seedUser({ username: "bob" });
    const otherFile = await seedFile({ ownerId: other.id, parentId: null, name: "bobs.jpg" });
    await seedShare({ nodeId: otherFile.id, createdBy: other.id, token: "bobs-tok" });
    const { response, data } = await callRoute<{ shares?: unknown[]; error?: string }>(listShares, {
      method: "GET",
      params: { id: otherFile.id },
      cookies: { doma_session: token },
    });
    // Ownership is checked first — Alice must not learn that Bob's file exists.
    expect(response.status).toBe(404);
...
describe("POST /api/share/[token] (verify share)", () => {
  ...
  beforeEach(async () => {
    ...
    fileId = f.id;
    const share = await seedShare({ nodeId: fileId, createdBy: user.id, token: "test-tok-123" });
  ...
  it("does NOT re-increment usedCount when the viewed cookie is present (refresh-safe)", async () => {
    const { shareViewedCookieKey } = await import("@/lib/auth/share-cookie");
    await db.share.update({
      where: { token: shareToken },
      data: { maxViews: 5, usedCount: 0 },
    });

    const first = await callRoute<{ share: { usedCount: number } }>(verifyShare, {
      method: "POST",
      params: { token: shareToken },
      body: {},
    });
    expect(first.response.status).toBe(200);
    expect(first.data!.share.usedCount).toBe(1);

    const viewedKey = shareViewedCookieKey(shareToken);
    const second = await callRoute<{ share: { usedCount: number } }>(verifyShare, {
      method: "POST",
      params: { token: shareToken },
      body: {},
      cookies: { [viewedKey]: "1" },
    ...
    expect(second.response.status).toBe(200);
    // Response echoes the non-incremented count when already viewed.
    expect(second.data!.share.usedCount).toBe(1);

    const row = await db.share.findUnique({ where: { token: shareToken } });
...
describe("GET /api/files/download/[id]", () => {
  ...
  beforeEach(async () => {
    ...
    fileId = f.id;
    const share = await seedShare({ nodeId: fileId, createdBy: user.id, token: "dl-tok-123" });
    shareToken = share.token;
  });
  ...
  it("blocks direct download for capped shares without a verify cookie", async () => {
    await db.share.update({
      where: { token: shareToken },
      data: { maxViews: 5, oneTimeUse: false },
    });
    const { response, data } = await callRoute<{ needsVerify?: boolean }>(downloadFile, {
      method: "GET",
      url: `http://localhost:3000/api/files/download/${fileId}?token=${shareToken}`,
      params: { id: fileId },
    });
    expect(response.status).toBe(403);
    expect(data!.needsVerify).toBe(true);
  });

  it("allows download for capped shares after verify sets the viewed cookie", async () => {
    const { shareViewedCookieKey } = await import("@/lib/auth/share-cookie");
    await db.share.update({
      where: { token: shareToken },
      data: { maxViews: 5, usedCount: 1 },
    });
    const { response } = await callRoute(downloadFile, {
      method: "GET",
      url: `http://localhost:3000/api/files/download/${fileId}?token=${shareToken}`,
      params: { id: fileId },
      cookies: { [shareViewedCookieKey(shareToken)]: "1" },
    });
    expect(response.status).toBe(200);
  });

  it("blocks direct download for oneTimeUse shares without verify", async () => {
    await db.share.update({
      where: { token: shareToken },
      data: { oneTimeUse: true, maxViews: null, usedCount: 0 },
    });
    const { response } = await callRoute(downloadFile, {
      method: "GET",
      url: `http://localhost:3000/api/files/download/${fileId}?token=${shareToken}`,
  ...
  it("returns 403 when the share token doesn't match the file", async () => {
    ...
    const otherShare = await seedShare({ nodeId: f2.id, createdBy: user.id, token: "other-tok" });
    const { response } = await callRoute(downloadFile, {
      method: "GET",
  ...
  it("returns 404 when downloading a directory", async () => {
    const folder = await seedFile({ ownerId: user.id, parentId: null, isDirectory: true, name: "folder" });
    const { response } = await callRoute(downloadFile, {
      method: "GET",
      params: { id: folder.id },
    ...
    // Directories are not downloadable; 404 (not 400) avoids confirming
    // that the id is a folder vs missing.
    expect(response.status).toBe(404);
  });
});
