...
describe("tree.ts DB functions", () => {
  ...
  describe("computeSubtreeSize", () => {
    ...
    it("sums ALL descendants (including soft-deleted) of a directory", async () => {
      const root = await seedFile({
        ownerId: user.id,
        parentId: null,
        isDirectory: true,
      ...
      // Note: computeSubtreeSize includes deleted files (unlike computeDirectorySize)
      await seedFile({
        ownerId: user.id,
        parentId: root.id,
        name: "b.txt",
      ...
      expect(size).toBe(30n); // 10 + 20, including the deleted one
    });
  });
  ...
  describe("purgeSubtree", () => {
    ...
    it("continues deleting DB rows even if storage.delete throws", async () => {
      ...
      const fakeStorage = {
        delete: async () => { throw new Error("S3 unavailable"); },
      };
