...
describe("useCloudStore (Zustand store)", () => {
  ...
  describe("setView", () => {
    ...
    it("switches to the 'shared-by-me' view with a 'Мои общие' root path", () => {
      useCloudStore.getState().setView("shared-by-me");
      const state = useCloudStore.getState();
      expect(state.view).toBe("shared-by-me");
      expect(state.path).toEqual([{ id: null, name: "Мои общие" }]);
    });
