...
export async function resetDb(): Promise<void> {
  // Order matters: delete child tables first (FK → FileNode / User), then FileNode, then User & Setting.
  // On SQLite TRUNCATE isn't available; DELETE is fast enough for tests.
  await db.share.deleteMany({});
  await db.sharedItem.deleteMany({});
...
export interface SeedSharedItemInput {
  nodeId: string;
  ownerId: string;
  recipientId: string;
...
/** @deprecated Prefer seedSharedItem — kept as alias for older tests. */
export type SeedSharedFolderInput = SeedSharedItemInput & {
  /** Alias for nodeId (folder shares). */
  folderId?: string;
};

/** Create a SharedItem ACL row (file or folder share with a user). */
export async function seedSharedItem(input: SeedSharedItemInput) {
  return db.sharedItem.create({
    data: {
      nodeId: input.nodeId,
      ownerId: input.ownerId,
...
/** @deprecated Prefer seedSharedItem. */
export async function seedSharedFolder(input: SeedSharedFolderInput) {
  return seedSharedItem({
    nodeId: input.nodeId ?? input.folderId!,
    ownerId: input.ownerId,
    recipientId: input.recipientId,
