...
/**
 * BirthdayGreeting — попап-поздравление в день рождения пользователя;
 * если birthday не задан — в годовщину создания аккаунта.
 * Закрытие запоминается в localStorage до конца локального дня.
 */
...
export function BirthdayGreeting({ user }: Props) {
  ...
  const greeting = mode === "birthday"
    ...
    : age === null
      ? "С праздником!"
      : `Сегодня аккаунту исполняется ${age} ${plural(age, "год", "года", "лет")}`;
