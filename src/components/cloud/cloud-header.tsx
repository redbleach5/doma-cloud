...
function DayGreeting({ displayName }: { displayName: string }) {
  ...
  return (
    <div className="min-w-0 hidden sm:flex flex-col justify-center leading-tight ml-1 mr-2">
      ...
      <span className="text-[11px] text-muted-foreground/80 truncate hidden md:block" title={g.sub}>
        {g.sub}
      </span>
    </div>
  );
...
export function CloudHeader({ user, onLogout }: Props) {
  ...
  const { data: meData } = useQuery({
    queryKey: ["me"],
    queryFn: () => api.me(),
    // Poll every 60s for fresh quota display. 30s was too frequent and
    // combined with cloud-app.tsx's focus-refetch caused double-fetches
    // every time the user switched tabs. 60s is a better tradeoff between
    // freshness and unnecessary requests.
    refetchInterval: 60_000,
    // Skip re-fetching on window focus — cloud-app.tsx already invalidates
    // the ["me"] query on focus, so re-fetching here would duplicate it.
