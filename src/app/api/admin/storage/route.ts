  ...
  if (!(await probeWritable(validated))) {
    const hint =
      process.platform === "win32"
        ? "Директория недоступна для записи — проверьте права доступа или защиту диска от записи."
        : process.platform === "darwin"
          ? "Директория недоступна для записи. Если это внешний диск на macOS — NTFS обычно только для чтения; используйте APFS/exFAT."
          : "Директория недоступна для записи — проверьте права доступа (chown/chmod) и не смонтирован ли диск в режиме read-only.";
