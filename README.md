...
- **Первый пользователь — администратор** с полным доступом и 3 ТБ квоты по умолчанию.
...
Первый пользователь, которого вы создадите, станет администратором с квотой 3 ТБ.
...
SQLite-база лежит в Docker volume `doma-db` — периодически делайте `docker compose exec doma bunx prisma db pull` или копируйте файл.
